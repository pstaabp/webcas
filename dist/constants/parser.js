...
export class Parser {
    ...
    static parseComplex(str) {
        ...
        if (m1 && m1[1] !== undefined)
            return new Complex(0, Parser.parseRealConstant(m1[1]));
        const m2 = imagRe2.exec(str);
        if (m2 && m2[2] !== undefined)
            return new Complex(0, Parser.parseRealConstant(m2[2]));
        const m3 = complexRe1.exec(str);
        if (m3 && m3[2] !== undefined)
            return new Complex(Parser.parseRealConstant(m3[1]), Parser.parseRealConstant(m3[2]));
        const m4 = complexRe2.exec(str);
        if (m4 && m4[2] !== undefined)
            return new Complex(Parser.parseRealConstant(m4[1]), Parser.parseRealConstant(m4[2]));
