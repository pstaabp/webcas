...
class Rational extends abstract_real_1.RealConstant {
    constructor(arg1, arg2, arg3) {
        super();
        let numer = BigInt(0), denom = BigInt(1);
        if (typeof arg1 !== 'string' && arg2 !== undefined) {
            if (!parser_1.constRe.integer.test(`${arg1}`))
                throw `The number ${arg1} is not an integer.`;
            if (!parser_1.constRe.integer.test(`${arg2}`))
                throw `The number ${arg2} is not an integer.`;
            numer = arg1 instanceof integer_1.Integer ? arg1.value : BigInt(arg1);
            denom = arg2 instanceof integer_1.Integer ? arg2.value : BigInt(arg2);
        }
        else if (typeof arg1 === 'string' && arg2 === undefined) {
            if (!parser_1.constRe.rational.test(arg1))
                throw arg1 + ' is not a rational number';
            const match = arg1.match(parser_1.constRe.rational);
            if (match !== null) {
    ...
    simplify() {
        if (this._denom == 1n)
            return new integer_1.Integer(this._numer);
        if (this._numer == 0n)
    ...
    toReal() {
        return new real_1.Real(Number(this._numer) / Number(this._denom));
    }
    equals(num) {
    ...
    plus(arg) {
        ...
        if (num instanceof integer_1.Integer)
            ...
            result = new real_1.Real(num.value + this.toReal().value);
    ...
    minus(arg) {
        ...
        if (num instanceof integer_1.Integer)
            ...
            result = new real_1.Real(this.toReal().value - num.value);
        else if (num instanceof Rational)
            result = new Rational(num.denom * this._numer - num.numer * this._denom, num.denom * this._denom);
        else if (num instanceof complex_1.Complex)
            result = new complex_1.Complex(this.minus(num.real), num.imag.times(new integer_1.Integer(-1)));
        else
            throw new Error("This line shouldn't be reached.");
        return result.simplify();
    }
    ...
    div(arg) {
        ...
        if (num instanceof integer_1.Integer)
            ...
            return new complex_1.Complex(this.times(num.real).div(d), this.times(num.imag).neg().div(d));
        }
        else
    ...
    reduce() {
        const c = integer_1.Integer.gcd(this._numer, this.denom);
        this._numer /= c;
        this._denom /= c;
    }
}
