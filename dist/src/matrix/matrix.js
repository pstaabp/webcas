...
exports.RowVector = exports.ColumnVector = exports.Matrix = void 0;
const abstract_constant_ts_1 = require("../constants/abstract_constant.ts");
...
class Matrix {
        ...
        if (typeof arg1 == 'number' && typeof arg2 === 'number') {
            ...
            for (let i = 0; i < arg1; i++) {
                ...
                for (let j = 0; j < arg2; j++)
                    this._arr[i][j] = new integer_ts_1.Integer(0);
            }
        }
            ...
            for (let i = 0; i < s.length; i++) {
                ...
                for (let j = 0; j < s2.length; j++) {
                    if (s2[j] != '')
                        row[row.length] = parser_ts_1.Parser.parseConstant(s2[j]);
                }
                this._arr[i] = row;
            }
        }
            ...
            for (let i = 0; i < arg1.length; i++) {
                this._arr[i] = arg1[i].map((el) => (el instanceof abstract_constant_ts_1.Constant ? el : parser_ts_1.Parser.parseConstant(`${el}`)));
            }
        }
    ...
    setElement(row, col, value) {
        ...
        if (typeof value === 'number' || typeof value === 'bigint' || typeof value === 'string')
            this._arr[row - 1][col - 1] = parser_ts_1.Parser.parseConstant(`${value}`);
        else
    ...
    multiplyRowByAndAdd(num, row, dest_row) {
        return this.rowCombination(num, row, new integer_ts_1.Integer(1), dest_row, dest_row);
    }
    ...
    pivot(row, col) {
        ...
        if (this.getElement(row, col).equals(integer_ts_1.Integer.ZERO))
            throw new Error('The pivot element cannot be zero.');
        this.multiplyRowBy(new integer_ts_1.Integer(1).div(this.getElement(row, col)), row);
        for (let i = 1; i <= this.nrow(); i++) {
            if (i != row)
    ...
    pivotPreserveIntegers(row, col) {
        if (this.getElement(row, col).compareTo(integer_ts_1.Integer.ZERO) < 0)
            this.multiplyRowBy(new integer_ts_1.Integer(-1), row);
...
class Vector extends Matrix {
    dot(v) {
        let result = integer_ts_1.Integer.ZERO;
...
class RowVector extends Vector {
    ...
    times(arg) {
        if (arg instanceof abstract_constant_ts_1.Constant) {
