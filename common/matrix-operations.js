...
function Matrix(str) {
  ...
  if (arguments.length == 1) {
    ...
    for (var i = 0; i < s.length; i++) {
      ...
      for (var j = 0; j < s2.length; j++) {
        if (s2[j] != "") row[row.length] = Mnumber.parseConstant(s2[j]);
      }
      arr[i] = row;
    }
  ...
  if (arguments.length == 2) {
    ...
    for (var j = 0; j < arr.length; j++) {
      ...
      for (var i = 0; i < arr[0].length; i++) arr[j][i] = new Integer(0);
    }
  }
  this.arr = arr;
}
...
Matrix.prototype.toPlainText = function () {
  var str = "";
  for (var j = 0; j < this.arr.length; j++) {
    for (var i = 0; i < this.arr[j].length; i++) str += this.arr[j][i] + " ";
    str += "\n";
  }
...
Matrix.prototype.operate = function (rowOp) {
  if (rowOp instanceof RowSwap) return this.swapRows(rowOp.row1 - 1, rowOp.row2 - 1);
    ...
    return this.multiplyRowBy(rowOp.row1 - 1, rowOp.factor1, rowOp.row2 - 1, rowOp.factor2, rowOp.row3 - 1);
...
Matrix.prototype.pivot = function (row, col) {
  ...
  for (var i = 0; i < this.arr.length; i++)
    if (i != row) m = m.multiplyRowBy(row, new Multiply(new Integer(-1), this.arr[i][col]), i, Integer.ONE);
...
Matrix.prototype.pivotPreserveIntegers = function (row, col) {
  ...
  if (m.SMMultiplier === undefined) {
    ...
    for (var k = 0; k < this.arr.length; k++) {
      ...
      m = m.multiplyRowBy(
        row,
        new Multiply(new Integer(-1), new Divide(m.arr[k][col], m.SMMultiplier)),
        k,
        new Divide(new Multiply(m.arr[row][col]), m.SMMultiplier)
      );
    }
  }
...
Matrix.prototype.multiplyRowBy = function (row1, num1, row2, num2, row3) {
  ...
  for (var j = 0; j < this.arr[row2].length; j++) {
    var tmp = new Add(new Multiply(this.arr[row1][j], num1), new Multiply(this.arr[row2][j], num2));
...
Matrix.prototype.clone = function () {
  ...
  for (i = 0; i < this.arr.length; i++) for (j = 0; j < this.arr[i].length; j++) m.arr[i][j] = this.arr[i][j];
...
Matrix.prototype.transpose = function () {
  ...
  for (i = 0; i < m.arr.length; i++) for (j = 0; j < m.arr[i].length; j++) m.arr[i][j] = this.arr[j][i];
...
Matrix.prototype.augment = function (B) {
  ...
  for (i = 0; i < this.arr.length; i++) {
    for (j = 0; j < this.arr[i].length; j++) m.arr[i][j] = this.arr[i][j];
    for (j = 0; j < B.arr[0].length; j++) m.arr[i][this.arr[0].length + j] = B.arr[i][j];
  }
  return m;
};
...
Matrix.prototype.plus = function (B) {
  if (!this.equalDimensions(B)) throw "Matrices are not the same size for addition";

  var m = new Matrix(this.arr.length, this.arr[0].length);

  for (var j = 0; j < this.arr.length; j++)
    for (var i = 0; i < this.arr[0].length; i++) m.arr[j][i] = new Add(this.arr[j][i], B.arr[j][i]);
...
Matrix.prototype.minus = function (B) {
  ...
  for (var j = 0; j < this.arr.length; j++)
    for (var i = 0; i < this.arr[0].length; i++) m.arr[j][i] = new Subtract(this.arr[j][i], B.arr[j][i]);
...
Matrix.prototype.times = function (B) {
  if (B instanceof Matrix) {
    if (this.arr[0].length != B.arr.length) throw "Matrices are not compatible for multiplication";

    var m = new Matrix(this.arr.length, B.arr[0].length);
    for (var j = 0; j < this.arr.length; j++)
      for (var i = 0; i < B.arr[0].length; i++) {
        var ops = new Array();
        for (var k = 0; k < B.arr.length; k++) ops[k] = new Multiply(this.arr[j][k], B.arr[k][i]);
    ...
    for (var j = 0; j < this.arr.length; j++)
      for (var i = 0; i < this.arr[0].length; i++) m.arr[j][i] = new Multiply(this.arr[j][i], B);
...
Matrix.prototype.power = function (n) {
  ...
  if (!this.isSquare()) throw "The matrix must be square in order to raise it to a power";

  var m = Matrix.identity(this.arr.length);

  for (var i = 0; i < n; i++) m = m.times(this);
...
Matrix.identity = function (n) {
  ...
  for (var i = 0; i < n; i++) m.arr[i][i] = new Integer(1);
...
Matrix.prototype.isRational = function () {
  for (i = 0; i < this.arr.length; i++)
    for (j = 0; j < this.arr[i].length; j++)
      if (!(this.arr[i][j] instanceof Integer || this.arr[i][j] instanceof Rational)) return false;
...
Matrix.prototype.isReal = function () {
  for (i = 0; i < this.arr.length; i++)
    for (j = 0; j < this.arr[i].length; j++) if (this.arr[i][j] instanceof Real) return true;
...
Matrix.prototype.rowReduceRational = function () {
  ...
  for (var col = 0; col < m.arr[0].length; col++) {
    ...
    if (!columnOfZeros) {
      ...
      var rat = null;
      if (m.arr[row][col] instanceof Integer) rat = new Rational(1, m.arr[row][col].value);
      else if (m.arr[row][col] instanceof Rational) rat = new Rational(m.arr[row][col].bottom, m.arr[row][col].top);
      ...
      for (var j = 0; j < m.arr.length; j++) {
        if (j != row)
          // eliminate to get zeros in all other positions.
...
Matrix.prototype.simplify = function () {
  ...
  for (var j = 0; j < this.arr.length; j++)
    for (var i = 0; i < this.arr[0].length; i++) m.arr[j][i] = this.arr[j][i].simplify();
...
Matrix.prototype.determinant = function () {
  if (!this.isSquare()) throw "Error in taking determinant.  The matrix is not square.";
  ...
  for (var col = 0; col < m.arr[0].length; col++) {
    ...
    if (!columnOfZeros) {
      ...
      for (var j = row + 1; j < m.arr.length; j++) {
        var rat =
          m.arr[j][col] instanceof Integer
...
Matrix.prototype.minor = function (i, j) {
  ...
  for (var k1 = 0; k1 < i; k1++) {
    for (var k2 = 0; k2 < j; k2++) m.arr[k1][k2] = this.arr[k1][k2];
    for (var k2 = j + 1; k2 < this.arr[0].length; k2++) m.arr[k1][k2 - 1] = this.arr[k1][k2];
  }
  for (var k1 = i + 1; k1 < this.arr.length; k1++) {
    for (var k2 = 0; k2 < j; k2++) m.arr[k1 - 1][k2] = this.arr[k1][k2];
    for (var k2 = j + 1; k2 < this.arr[0].length; k2++) m.arr[k1 - 1][k2 - 1] = this.arr[k1][k2];
  }
...
Matrix.prototype.det2 = function () {
  ...
  if (this.arr.length == 2) {
    result = new Add(
      new Multiply(this.arr[0][0], this.arr[1][1]),
      new Multiply(new Integer(-1), this.arr[0][1], this.arr[1][0])
    );
  } else {
    var ops = new Array();
    for (var i = 0; i < this.arr.length; i++) {
      if (i % 2 == 0) ops[i] = new Multiply(this.arr[0][i], this.minor(0, i).det2());
      else ops[i] = new Multiply(new Integer(-1), this.arr[0][i], this.minor(0, i).det2());
...
Matrix.prototype.invert = function () {
  if (!this.isSquare()) throw "Error in taking inverse.  The matrix is not square.";

  var m = new Matrix(this.arr.length, 2 * this.arr[0].length);

  for (var j = 0; j < m.arr.length; j++) {
    for (var i = 0; i < this.arr[0].length; i++) m.arr[j][i] = this.arr[j][i];
  ...
  if (!m.arr[m.arr.length - 1][m.arr.length - 1].equals(Integer.ONE)) exists = false;

  if (!exists) throw "The inverse of the matrix does not exist";

  for (var j = 0; j < minv.arr.length; j++) {
    for (var i = 0; i < minv.arr[0].length; i++) minv.arr[j][i] = m.arr[j][i + minv.arr.length];
  }
...
ElementaryRowOperation.parseString = function (str) {
  ...
  if (result2 != null || result3 != null) {
    ...
    if (row1 == row2) throw "Error in Swapping rows: the two rows must be unique.";
  ...
  if (result != null) {
    if (result[1] == undefined) {
      // has form 2R2->R2;
      if (result[5] == "-") num2 = new Integer(-1);
      else if (result[4] == "-") num2 = new Multiply(Mnumber.parseConstant(result[5]), new Integer(-1));
      else num2 = Mnumber.parseConstant(result[5]);

      row2 = Number(result[6]);
      if (result[8] == undefined) row3 = row2;
      else row3 = Number(result[8]);
    ...
    if (result[2] == "") num1 = new Integer(1);
    else if (result[2] == "-") num1 = new Integer(-1);
    else num1 = Mnumber.parseConstant(result[2]);
    row1 = Number(result[3]);

    if (result[5] == undefined || result[5] == "") {
      if (result[4] == "-") num2 = new Integer(-1);
      else num2 = new Integer(1);
    } else {
      if (result[4] == "-") num2 = new Multiply(Mnumber.parseConstant(result[5]), new Integer(-1));
      else num2 = Mnumber.parseConstant(result[5]);
    }
...
RowMultiplyAndAdd.prototype.toString = function () {
  return "Mult row " + this.row1 + " by " + this.factor1 + " and add to " + this.factor2 + " times row " + this.row3;
};
...
RowMultiplyAndAdd.prototype.toLaTeX = function () {
  ...
  if (this.factor1 instanceof Integer && this.factor1.value == -1) row1 = "-" + r1;
  else if (this.factor1 instanceof Integer && this.factor1.value == 1) row1 = r1;
  else row1 = this.factor1.toLaTeX() + r1;
  ...
  if (fact2 instanceof Integer && fact2.value == 1) row2 = r2;
  else row2 = fact2.toLaTeX() + r2;
...
ToDecimal.prototype.toString = function () {
  return "ToDecimal";
};

ToDecimal.prototype.toLaTeX = function () {
  return "\\mbox{The matrix converted to decimals is:}";
};
